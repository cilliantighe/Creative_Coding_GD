...
function draw() {
  ...
  if (displayText && textAlpha > 0) {
    ...
    textSize(40);
    textFont(descFont);
    text('Colour Palettes From Rules: Shape Pattern', width / 2, height / 1.8);
    ...
    textAlign(LEFT);
    fill(0, 0, 100);
    text('06_04_shape_pattern', 25, 30);
  }
}
...
function keyPressed() {
  if (key == 's' || key == 'S') saveCanvas(canvas, '06_04_shape_pattern', 'png');
}
