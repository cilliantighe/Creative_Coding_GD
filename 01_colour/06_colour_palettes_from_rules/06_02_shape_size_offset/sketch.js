...
function draw() {
  ...
  for (var i = 0; i < colourCount; i++) {
    if (i % 2 == 0) {
      hueValues[i] = floor(random(130, 220));
      saturationValues[i] = 100;
      brightnessValues[i] = floor(random(15, 100));
    } else {
      hueValues[i] = 195;
      saturationValues[i] = floor(random(20, 100));
  ...
  if (displayText && textAlpha > 0) {
    ...
    textSize(40);
    textFont("Roboto");
    text('Colour Palettes From Rules: Size Offset', width / 2, height / 1.8);
    ...
    textAlign(LEFT);
    fill(0, 0, 100);
    text('06_02_shape_size_offset', 25, 30);
  }
}
...
function keyPressed() {
  if (key == 's' || key == 'S') saveCanvas(canvas, '06_02_shape_size_offset', 'png');
}
