...
function draw() {
  ...
  for (var y = 0; y < txtG.height; y += pixelD) {
    for (var x = 0; x < txtG.width; x += pixelD) {
      ...
      if (txtG.pixels[graphicIndex] <= 128) {
        fill(img.pixels[imgIndex], img.pixels[imgIndex+1], img.pixels[imgIndex+2]);
        ellipse(x, y, 10, 10);
      }
    }
  }
  ...
  if (textAlpha > 0) {
    ...
    textSize(40);
    textFont("Roboto");
    text('Image Overlay', width / 2, height / 1.8);
...
function keyPressed() {
  if (keyCode === DOWN_ARROW) saveCanvas(canvas, '03_image_overlay', 'png');
}
